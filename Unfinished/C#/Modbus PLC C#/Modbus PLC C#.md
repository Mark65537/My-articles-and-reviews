...
todo добавить что используется(CODESYS, NModbus, Modbus Slave, Modbus TCP)
...
3. обмен в случае Modbus TCP/IP выполняется поверх TCP
4. каждое устройство имеет уникальный Slave ID

>[!NOTE]
Если в качестве сервера используется программа Modbus Slave, чтение регистров приходится начинать с 10-го адреса — это практическая особенность используемого инструмента или его реализации протокола.
...
Для работы с Modbus в C# нужно использовать какую нибуть библиотеку.
Я использовал библиотеку Nmodbus. потому что, эта первая библиотека, которую я встретил. Я посмотрел, что на ней просто работать, начал использовать ее. Со временем, конечно я узнал о проблеме чления больше 125 регистра. Я посмотрел есть ли библиотека которая делает мульти запрос для нужного значения регистров в документации библиотеки [PL.Modbus](https://www.nuget.org/packages/PL.Modbus) было написанно что она может это делать. Если хотите можете использовать ее, но я ее не использовал не могу сказать как она работает. О том почему я не перешел на другую библиотеку вы можете прочитать в [Заключение](##Заключение).
...
Для чтения данных по Modbus просто используется функция `ushort[] ReadHoldingRegisters(byte slaveAddress, ushort startAddress, ushort numberOfPoints);`.
