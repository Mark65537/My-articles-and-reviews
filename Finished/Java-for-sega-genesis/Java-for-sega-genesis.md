...
Java Grinder изначально был сделан для линукса, и на данный момент есть только зачатки [порта для windows](https://github.com/Mark65537/Java_Grinder-Windows), поэтому либо придётся использовать линукс, либо [WSL](https://learn.microsoft.com/ru-ru/windows/wsl/install). Я использовал [WSL](https://learn.microsoft.com/ru-ru/windows/wsl/install), поэтому все дальнейшие примеры буду приводить на нем. Чтобы начать создавать свои проекты необходимо выполнить несколько шагов:

1. Установите в ваш [WSL](https://learn.microsoft.com/ru-ru/windows/wsl/install) утилиту `make` для сборки проектов и `javac`(openjdk) для компиляции java-файлов.
...
<picture>
  <source srcset="./images/Lemming.png">
  <img src="https://raw.githubusercontent.com/Mark65537/My-articles-and-reviews/refs/heads/main/Finished/Java-for-sega-genesis/images/Lemming.png" alt="Lemming">
</picture>
...
```java
int color9bit = ((color.B >> 5) << 9) | ((color.G >> 5) << 5) | ((color.R >> 5) << 1);
```

К сожалению, точное обратное преобразование получить почти невозможно, так как на одно значение приходится 32 RGB-цвета. Вы можете воспользоваться данным кодом для обратного преобразования, но он не гарантирует, что вы получите такие же цвета, как на эмуляторе или железе.

```java
int b = (color9bit >> 9) & 0x7;
int g = (color9bit >> 5) & 0x7;
int r = (color9bit >> 1) & 0x7;
Color rgb = new Color(r << 5, g << 5, b << 5);
```
...
1. массив palette из [предыдущего раздела.](#палитра)  
2. массив pattern  
...
<picture>
  <source srcset="./images/JavaGrinderLogo.jpg">
  <img src="https://raw.githubusercontent.com/Mark65537/My-articles-and-reviews/refs/heads/main/Finished/Java-for-sega-genesis/images/JavaGrinderLogo.jpg" alt="JavaGrinderLogo">
</picture>
...
<picture>
  <source srcset="./images/mouse-parce.png">
  <img src="https://raw.githubusercontent.com/Mark65537/My-articles-and-reviews/refs/heads/main/Finished/Java-for-sega-genesis/images/mouse-parce.png" alt="mouse-parce">
</picture>
...
На данный момент реализовано только 3-кнопочное управление, без кнопки Mode. В API содержится метод для получения кода текущей нажатой кнопки `SegaGenesis.getJoypadValuePort1` и константы кодов кнопок. Для удобства работы с этим методом в коде определены константы, которые соответствуют кодам кнопок:
...
<picture>
  <source srcset="./images/Shema.png">
  <img src="https://raw.githubusercontent.com/Mark65537/My-articles-and-reviews/refs/heads/main/Finished/Java-for-sega-genesis/images/Shema.png" alt="">
</picture>
...
Есть 3 способа как проиграть мелодию: загрузить звук в `Z80 RAM` плюс код инициализации, загрузить ноты и их последовательность в `Z80 RAM` плюс код инициализации, загрузить данные из `.wav` файла в `ROM` и `.wav` проигрыватель в `Z80 RAM` и передать адрес начала в определенное смещение в `Z80 RAM`. Разберем каждый способ по порядку

**Способ 1. Загрузить звук с кодом инициализации в `Z80 RAM`.**  
Для начала нам понадобиться код инициализации. Код инициализации это код на ассемблере `Z80`, который передается через процессор `M68000` в `Z80 RAM` как скомпилированный массив данных. Чтобы его получить вы можете скомпилировать файл [z80_play_dac.asm](https://github.com/mikeakohn/java_grinder/blob/master/samples/sega_genesis/z80_play_dac.asm) с помощью `naked_asm`, перевести его в java байт массив и выделить из него код инициализации, или можете использовать готовый массив кода инициализации:  
...
Автор Java Grinder для реализации данного способа, использовал гитарные аккорды и проигрывал их в цикле по заданному порядку. Можете модифицировать данный [код](https://github.com/mikeakohn/java_grinder/blob/master/samples/sega_genesis/z80_play_title_song.asm) для создания своей собственной мелодии, после чего скомпилировать его с помощью `naked_asm` и перевести в java байт массив.
...
1. [sega_genesis_java_demo.bin](https://www.mikekohn.net/micro/binaries/sega_genesis_java_demo.bin) - это демо версия от разработчика для демонстрации возможности движка  
...
1. если выдает ошибку Couldn't find “ИмяПоля”  
   ...
   ```text
   ** Error setting statics ../common/JavaCompiler.cxx:2416.
   ```

   Попробуйте сделать поле `final`.  
